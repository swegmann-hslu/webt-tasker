...
function getAndValidateBody(req) {
    ...
    if (!body.hasOwnProperty('username')) throw createMessage(RETVAL_VALIDATION_FAILED, 'property "username" is missing');
    ...
    if (!body.hasOwnProperty('password')) throw createMessage(RETVAL_VALIDATION_FAILED, 'property "password" is missing');
...
function handleError(res, error) {
    if (error.hasOwnProperty('id') && error.hasOwnProperty('message')) {
        res.status(400);
